...
import { ProductAttributeInListDto, ProductAttributeService } from '@proxy/catalog/product-attributes';
...
export class ProductAttributeComponent implements OnInit, OnDestroy {
  ...
  selectAttribute(event: any): void {
  ...
  deleteItemsConfirmed(attribute: ProductAttributeValueDto, id: string) {
    this.toggleBlockUI(true);
    this.productService.removeProductAttribute(attribute.attributeId, id).pipe(takeUntil(this.ngUnsubscribe)).subscribe({
      next: () => {
  ...
  getValueByType(attribute: ProductAttributeValueDto) {
    if (attribute.dataType == AttributeType.Date) {
